class RedBlackTree:
    ...
    def insert(self, word, embedding):
        new_node = RBTNode(word, embedding, None, True, None, None)
    ...
    def _bst_remove_node(self, node):
        ...
        if node.left is not None and node.right is not None:
            ...
            # Copy successor's word
            successor_word = successor_node.word
            successor_embedding = successor_node.embedding

            # Recursively remove successor
            self._bst_remove_node(successor_node)

            # Set node's word to copied successor word
            node.word = successor_word
    ...
    def remove_node(self, node):
        if node.left is not None and node.right is not None:
            ...
            predecessor_word = predecessor_node.word
            self.remove_node(predecessor_node)
            node.word = predecessor_word
            return

        if node.is_black():
            self.prepare_for_removal(node)
        self._bst_remove(node.word)
    ...
    def search(self, word):
        current_node = self.root
        while current_node is not None:
            # Return the node if the word matches.
            if current_node.word == word:
                return current_node
        
            # Navigate to the left if the search word is
            # less than the node's word.
            elif word < current_node.word:
                current_node = current_node.left

            # Navigate to the right if the search word is
            # greater than the node's word.
            else:
                current_node = current_node.right

        # The word was not found in the tree.
        return None
