class AVLTree:
	...
	def remove_node(self, node):
		...
		if node.left is not None and node.right is not None:
			...
			node.word = successor_node.word
			node.embedding = successor_node.embedding
	...
	# Searches for a node with a matching word. Does a regular
	# binary search tree search operation. Returns the node with the
	# matching word if it exists in the tree, or None if there is no
	# matching word in the tree.
	def search(self, word):
		current_node = self.root
		while current_node is not None:
			# Compare the current node's word with the target word.
			# If it is a match, return the current word; otherwise go
			# either to the left or right, depending on whether the 
			# current node's word is smaller or larger than the target word.
			if current_node.word == word: return current_node
			elif current_node.word < word: current_node = current_node.right
			else: current_node = current_node.left

	# Attempts to remove a node with a matching word. If no node has a matching word
